...
export async function revalidateMonitorNativeRuntime(input: {
    ...
    assertCommand(activate, result.selection.preset === "analytics" ? "activate-analytics-server" : "activate-monitor-server", { unit: "rz-full.service", config: result.selection.preset === "analytics" ? ["/opt/rz/config/rz-admin.env", "/opt/rz/config/rz-insights.env"] : ["/opt/rz/config/rz-admin.env", "/opt/rz/config/rz-monitor.env"] });
...
function bindMarkers(publication: Json, activation: Json, result: MonitorNativeRuntimeEvidence) {
    ...
    const units = result.selection.preset === "analytics" ? ["rz-admin.service", "rz-insights.service", "rz-full.service"] : ["rz-admin.service", "rz-monitor.service", "rz-full.service"];
...
async function bindNotificationIngress(path: string | undefined, result: MonitorNativeRuntimeEvidence) {
    if (result.selection.preset === "analytics") {
        if (path) fail("analytics runtime evidence must not include notification ingress");
        return;
    }
    if (!path) fail("notification ingress evidence is required");
