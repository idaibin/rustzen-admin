...
test("captured Monitor snapshot publishes and rereads an immutable signed triplet", async () => {
    ...
    const trusted = { keyId: "captured-test", publicKey: keys.publicKey.export({ type: "spki", format: "pem" }).toString() };
    try {
        ...
        const result = await publishMonitorContainerRelease({
            snapshot, outputParent: join(trustedRoot, "output"), trustedRoot,
            privateKey: keys.privateKey.export({ type: "pkcs8", format: "pem" }).toString(), trusted,
        });
        expect(result.manifest.buildId).toBe(result.buildId);
        expect((await verifyReleaseSnapshot(result.root, selection, trusted)).manifest).toEqual(result.manifest);
        expect((await Bun.file(join(result.root, "archive.tar")).stat()).mode & 0o777).toBe(0o644);
        await expect(publishMonitorContainerRelease({
            snapshot, outputParent: join(trustedRoot, "output"), trustedRoot,
            privateKey: keys.privateKey.export({ type: "pkcs8", format: "pem" }).toString(), trusted,
...
test("captured release rejects a valid replacement after publication", async () => {
    ...
    const trusted = { keyId: "reread-test", publicKey: keys.publicKey.export({ type: "spki", format: "pem" }).toString() };
    const fixture = await serverManifestFixture();
    try {
        const alternate = await publishSelectedRelease({
            selection, staging: fixture.staging, manifest: fixture.manifest, privateKey, trusted,
        });
        const snapshot = await verifyContainerExport(exportRoot, selection, sourceIdentity, releaseVersion);
        ...
        await expect(publishMonitorContainerRelease({
            snapshot, outputParent: join(trustedRoot, "output"), trustedRoot, privateKey, trusted,
...
test("invalid key material fails before captured staging can create output", async () => {
    const exportRoot = await createExport();
    const trustedRoot = await mkdtemp(join(tmpdir(), "rz-captured-invalid-key-"));
    ...
    try {
        for (const trusted of [
            { keyId: "bad key", publicKey: first.publicKey.export({ type: "spki", format: "pem" }).toString() },
            { keyId: "mismatch", publicKey: second.publicKey.export({ type: "spki", format: "pem" }).toString() },
            { keyId: "rsa", publicKey: rsa.publicKey.export({ type: "spki", format: "pem" }).toString() },
        ]) {
            await expect(publishMonitorContainerRelease({ snapshot, outputParent, trustedRoot, privateKey, trusted })).rejects.toThrow();
