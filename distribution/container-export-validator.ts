...
export class VerifiedContainerExportSnapshot {
    ...
    private constructor(token: symbol, files: ArtifactFile[], manifest: OutputManifest, provenance: Provenance) {
        if (token !== snapshotConstructionToken) throw new Error("verified snapshot construction is internal");
        this.#files = new Map(
            files.map((file) => [
                file.entry.path,
                {
    ...
    static {
        createVerifiedContainerSnapshot = (files, manifest, provenance) => new VerifiedContainerExportSnapshot(snapshotConstructionToken, files, manifest, provenance);
    }
    ...
    artifact(path: string): ArtifactFile {
        ...
        if (sha256(file.bytes) !== file.entry.sha256) throw new Error(`verified snapshot digest differs: ${path}`);
        return {
            entry: structuredClone(file.entry),
            bytes: new Uint8Array(file.bytes),
        };
    ...
    artifacts(prefix: string): ArtifactFile[] {
        if (!prefix || prefix.startsWith("/") || prefix.includes("\\") || !prefix.endsWith("/")) throw new Error("verified snapshot artifact prefix is invalid");
        return this.paths()
            .filter((path) => path.startsWith(prefix))
            .map((path) => this.artifact(path));
    }
...
export async function verifyContainerExport(root: string, selectionInput: unknown, expectedSourceIdentity: string, expectedReleaseVersion: string): Promise<VerifiedContainerExportSnapshot> {
    ...
    const inventory = selectedServerInventory(plan);
    const serverPaths = inventory.binaries.map((path) => `release/server/${path}`);
    if (typeof expectedSourceIdentity !== "string" || !expectedSourceIdentity || /[\r\n]/.test(expectedSourceIdentity)) throw new Error("expected source identity must be nonempty single-line text");
    if (typeof expectedReleaseVersion !== "string" || !expectedReleaseVersion || expectedReleaseVersion.length > 64 || /[\r\n]/.test(expectedReleaseVersion)) throw new Error("expected releaseVersion must be nonempty single-line text");
    const files = await readArtifactFileTree(root, {
        maxFiles: 512,
    ...
    if (provenance.releaseVersion !== expectedReleaseVersion) throw new Error("container provenance releaseVersion differs from workspace version");
    ...
    exact(
        ...
        "container output manifest files",
    );
    exact(
        ...
        "container output selection",
    );
    if (canonicalJson(provenance.selection) !== canonicalJson(selectionInput) || provenance.selectionSha256 !== sha256(canonicalJson(selectionInput))) throw new Error("container provenance selection differs from input");
    ...
    exact(provenance.buildCommands, syntheticServerBuildCommands(plan), "container provenance build commands");
    if (provenance.outputManifestSha256 !== sha256(canonicalJson(manifest))) throw new Error("container provenance output manifest digest differs");
    exactPayload(payload, serverPaths, inventory.hasAgentWitness);
    ...
    for (const path of serverPaths.slice(1)) verifyMonitorExportElf(required(byPath, path).bytes, path.slice("release/server/bin/".length));
    if (inventory.hasAgentWitness) verifyMonitorExportElf(required(byPath, witnessPath).bytes, "rz-monitor-agent");
...
function verifyWeb(files: Map<string, ArtifactFile>, payload: ArtifactFile[], plan: ReturnType<typeof resolveSelection>) {
    ...
    if (canonicalJson(descriptor) !== new TextDecoder("utf-8", { fatal: true }).decode(descriptorBytes)) throw new Error("container Web binding is not canonical");
    if (canonicalJson(descriptor) !== canonicalJson(inventory.binding)) throw new Error("container Web binding differs from inventory");
    const actual = payload
        .filter((file) => file.entry.path.startsWith("release/web/dist/"))
        .map((file) => ({
            ...file.entry,
            path: file.entry.path.slice("release/web/dist/".length),
            bytes: file.bytes,
        }))
    ...
    verifyWebBinding({
        binding: descriptor,
        compositionId: plan.compositionId,
        selectedApiBytes: apiSource,
        files: actual,
    });
    const text = payload
        .filter((file) => file.entry.path.startsWith("release/web/dist/") && /\.(?:html|js|css|map)$/.test(file.entry.path))
...
function parseProvenance(bytes: Uint8Array): Provenance {
    ...
    if (!Array.isArray(buildCommands) || buildCommands.some((command) => !Array.isArray(command) || command.some((part) => typeof part !== "string" || !part))) throw new Error("container provenance buildCommands is invalid");
    const result = {
        ...value,
        sourceIdentityInput: text(value.sourceIdentityInput, "container provenance sourceIdentityInput"),
        rustcVv: text(value.rustcVv, "container provenance rustcVv"),
        buildCommands,
    } as Provenance;
...
function parseFiles(value: unknown): ExportFile[] {
    ...
    const parsed = value.map((entry) => {
        ...
        if (!path || path.startsWith("/") || path.includes("\\") || path.split("/").some((part) => !part || part === "." || part === "..")) throw new Error("container output manifest file path is invalid");
        ...
        return {
            ...
            sha256: validHash(text(record.sha256, "container output manifest file sha256")),
        } as ExportFile;
    });
    if (canonicalJson(parsed) !== canonicalJson(value) || new Set(parsed.map((entry) => entry.path)).size !== parsed.length || canonicalJson(parsed) !== canonicalJson(parsed.slice().sort((left, right) => compareContainerExportPath(left.path, right.path)))) throw new Error("container output manifest files must be sorted and unique");
    return parsed;
}
function exactPayload(payload: ArtifactFile[], serverPaths: string[], hasWitness: boolean) {
    const expected = [...serverPaths, ...(hasWitness ? [witnessPath] : []), ...contractPaths];
    for (const file of payload) {
        const path = file.entry.path;
        if (path.startsWith("release/web/")) {
            if (!["release/web/inventory.json", "release/web/binding.json", "release/web/api.ts"].includes(path) && !path.startsWith("release/web/dist/")) throw new Error(`container export has unexpected Web payload path: ${path}`);
        } else if (!expected.includes(path)) {
            throw new Error(`container export has unexpected payload path: ${path}`);
        }
    }
    exact(
        payload.filter((file) => !file.entry.path.startsWith("release/web/")).map((file) => file.entry.path),
        expected.sort(),
        "container binary and contract inventory",
    );
...
function verifyModes(files: ArtifactFile[]) {
    const serverPaths = files.filter((file) => file.entry.path.startsWith("release/server/bin/")).map((file) => file.entry.path);
    for (const file of files) {
        const path = file.entry.path;
        const executable = serverPaths.includes(path) || path === witnessPath;
        const expected = executable ? "0755" : "0644";
        if (file.entry.mode !== expected) throw new Error(`container export file mode differs: ${path}`);
    }
}
...
function validHostSyntheticPlatform(value: string) {
    const parts = value.split("/");
    return parts.length === 3 && parts[0] === "host" && parts.slice(1).every((part) => /^[A-Za-z0-9._-]+$/.test(part));
}
function manifestIdentity(value: OutputManifest) {
    return {
        preset: value.preset,
...
function required(files: Map<string, ArtifactFile>, path: string) {
    const file = files.get(path);
    if (!file) throw new Error(`container export is missing ${path}`);
    return file;
}
function paths(files: ArtifactFile[]) {
    return files.map((file) => file.entry.path).sort();
}
...
function json(bytes: Uint8Array, label: string): Record<string, unknown> {
    try {
        ...
        throw new Error(`${label} is not JSON object`);
    }
}
