...
test("revalidator fails closed for certificate, process, marker and encoding changes", async () => {
    ...
    const activation = { adminConfigSha256: hash, buildId: build, monitorConfigSha256: hash, schemaFingerprint: { admin: hash, monitor: hash }, dataContractId: { admin: hash, monitor: hash }, state: "ready", unitSha256: { "rz-admin.service": hash, "rz-monitor.service": hash, "rz-full.service": hash }, version: 2 };
    ...
    const files: Record<string, unknown> = {
        evidence, admission: { archiveSha256: hash, binaryDigests: binary, buildId: build, certificateSha256: hash, envelopeSha256: hash, manifestSha256: hash, selection: { artifactClass: "server", compositionId: composition, preset: "monitor", target: "x86_64-unknown-linux-musl" } }, release: { archiveSha256: hash, buildId: build, envelopeSha256: hash, manifestSha256: hash, root },
        ...
        verify: envelope("verify", { build_id: build, artifact_class: "server", target: "x86_64-unknown-linux-musl", files: 1 }), dry: envelope("apply", { dry_run: true, release: releaseData(build) }), apply: envelope("apply", { dry_run: false, release: releaseData(build) }), status: envelope("install-status", { present: true, markerPresent: true, runnable: false, destination: "/opt/rz", current: `releases/${build}/payload` }), activate: envelope("activate-monitor-server", { unit: "rz-full.service", config: ["/opt/rz/config/rz-admin.env", "/opt/rz/config/rz-monitor.env"] }), publication, activation,
    };
    ...
    input.notificationIngressPath = join(root, "notification.check");
    const write = async () => { for (const [name, value] of Object.entries(files)) await writeFile(join(root, `${name}.json`), canonicalJson(value)); };
    try {
        evidence.checks.notificationIngress = "absent";
        await writeFile(input.notificationIngressPath, "absent\n");
        await write(); await expect(revalidateMonitorNativeRuntime(input)).resolves.toEqual(evidence);
        delete (files.admission as any).selection.preset;
        await write(); await expect(revalidateMonitorNativeRuntime(input)).rejects.toThrow("JSON schema differs");
        (files.admission as any).selection.preset = "monitor";
        ...
        (files.admission as any).certificateSha256 = hash; (files.admission as any).selection.preset = "monitor-notify"; await write(); await expect(revalidateMonitorNativeRuntime(input)).rejects.toThrow("evidence selection differs");
        (files.admission as any).selection.preset = "monitor"; (files.facts as any).adminPid = 9; await write(); await expect(revalidateMonitorNativeRuntime(input)).rejects.toThrow("runtime facts differ");
        ...
        await writeFile(input.evidencePath, canonicalJson(evidence)); evidence.selection.preset = "monitor-notify"; evidence.checks.notificationIngress = "unauthorized"; (files.admission as any).selection.preset = "monitor-notify"; (files.activation as any).state = "ready"; (files.activation as any).schemaFingerprint = { admin: hash, "admin-notifications": hash, monitor: hash, "monitor-notifications": hash }; (files.activation as any).dataContractId = { admin: hash, "admin-notifications": hash, monitor: hash, "monitor-notifications": hash }; evidence.markers.activationSha256 = sha256(canonicalJson(files.activation)); (files.facts as any).activationSha256 = evidence.markers.activationSha256; await write(); await writeFile(input.notificationIngressPath, "tampered\n"); await expect(revalidateMonitorNativeRuntime(input)).rejects.toThrow("notification ingress evidence differs"); await writeFile(input.notificationIngressPath, "unauthorized\n"); await expect(revalidateMonitorNativeRuntime(input)).resolves.toEqual(evidence);
