...
test("selected config accepts exact server and Agent descriptors", async () => {
    ...
    try {
        for (const [name, selection] of [
            ...
            const a = await produceSelectedConfig(
                selection,
                out,
                runnerFor(selection),
            );
            const b = await produceSelectedConfig(
                selection,
                out,
                runnerFor(selection),
            );
...
test("selected config rejects identity and every field metadata mutation", () => {
    const mutations: Array<(v: any) => void> = [
        ...
        (v) =>
            v.owners.access.fields.push(
                structuredClone(v.owners.access.fields[0]),
            ),
    ];
...
test("selected config requires canonical stable single-file bytes", async () => {
    ...
    try {
        ...
        await produceSelectedConfig(
            serverSelection,
            out,
            runnerFor(serverSelection),
        );
        const valid = canonicalJson(
            completeSelectedConfigForTest(serverSelection),
        );
