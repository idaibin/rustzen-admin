...
| D37 | Wrong-owner DB, dropped index, extra trigger, changed identity or pending migration | Read-only admission rejects without product schema/data/ledger mutation or normal startup |
...
| D55 | Pin signed Monitor Server and published Agent fixtures; alter signature, tuple, endpoint, Agent protocol, profile bytes or replace the profile with a link | Pinning fails closed before a profile write; a valid root-owned profile gates Agent startup before logging/network work |
