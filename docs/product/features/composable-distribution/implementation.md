...
| `analytics` | Explicit pure-Insights service build, selected Admin/Insights fresh schema, a composition-qualified selected Web artifact containing only access and Analytics routes/API imports, strict reviewed access/Insights config, canonical Admin/Insights API and delegation protocol plus native-layout producers; the Admin build verifies the exact Web and two-read API authority | Rejected | Complete Cargo producer; the completed service/Admin/Web/API/schema/config/protocol/native-layout slices do not admit the server producer family |
