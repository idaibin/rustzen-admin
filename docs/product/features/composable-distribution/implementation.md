...
| P4 | Monitor native producer and payload publication | Composition-qualified Admin/Monitor/Agent binaries with selected Web, selected API/schema/config/protocol descriptors, signed exact-member tar, immutable payload publication verification | In implementation. Canonical manifest/archive/envelope production and `rz verify`/fresh-root `rz apply` publish an immutable non-runnable payload. A root-owned journal continues only the same revalidated archive/envelope/manifest tuple after a fresh-root publication interruption; target service start and DB initialization remain open. Monitor recovery is not a service, DeployService extraction, rollback or DB restore. |
