...
| `analytics` | Explicit pure-Insights service build, selected Admin source composition/fresh schema, a composition-qualified selected Web artifact containing only access and Analytics routes/API imports, and the strict reviewed access/Insights config producer; the Admin build verifies the exact Web and two-read API authority | Rejected | Complete Cargo, API, schema, protocol and native-layout producers; the completed service/Admin/Web/config slices do not admit the server producer family |
