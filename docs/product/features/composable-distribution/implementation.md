...
| `analytics` | Explicit pure-Insights service build, selected Admin source composition/fresh schema, a composition-qualified selected Web artifact containing only access and Analytics routes/API imports, strict reviewed access/Insights config, and canonical Admin/Insights native layout producers; the Admin build verifies the exact Web and two-read API authority | Rejected | Complete Cargo, API, schema and protocol producers; the completed service/Admin/Web/config/native-layout slices do not admit the server producer family |
