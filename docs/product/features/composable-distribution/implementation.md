...
| `analytics` | Catalog/application baseline plus an explicit `rz-insights --no-default-features --features selected-distribution` service build; its config dependency enables only the Insights owner | Rejected | Selected Admin Cargo boundary, Web graph and contracts; the service-only build does not admit the server producer family |
