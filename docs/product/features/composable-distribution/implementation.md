...
| `analytics` | Explicit pure-Insights service build, selected Admin source composition/fresh schema, a composition-qualified selected Web artifact containing only access and Analytics routes/API imports, strict reviewed access/Insights config, canonical Admin/Insights API and native-layout producers; the Admin build verifies the exact Web and two-read API authority | Rejected | Complete Cargo, schema and protocol producers; the completed service/Admin/Web/API/config/native-layout slices do not admit the server producer family |
