...
| `reports` | Catalog, Reports application, notification producer, and the selected-service Cargo closure for `rz-reports` (`selected-distribution`, config owner `reports` only, no notification transport); the service-only build does not admit the server producer family | Rejected | Selected Admin Cargo boundary, Web graph and contracts |
