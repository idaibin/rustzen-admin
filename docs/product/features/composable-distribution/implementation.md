...
| P7 | Direct Admin fetch-SSE and shell integration | Stream/parser/cancellation tests and real browser/proxy reconciliation | P7a auth/session authority and P7b backend SSE are **Closed locally / Passed** at source, independent-review, and disposable `linux/arm64` runtime layers. P7a persists random session IDs and auth epochs, reloads current session/user/grants for protected requests, repeats actor checks inside access writes, and invalidates prior epochs on logout, revoke-all, password, status, or grant changes. Its final manifest is `target/rz/admin-session-authority/current/manifest.json`. P7b adds the notifications-owned direct stream, path-only request logging, typed authority-store refusal for every protected operation, bounded hub quotas/queues, post-commit admission/read/retention invalidations, exact expiry, durable monotonic revisions, current-authority rechecks, lag reconciliation, a 45-second body-poll deadline, and a five-minute absolute connection age. The final P7b acceptance run `target/rz/admin-notification-sse/runs/20260907T222124Z-10050/manifest.json` binds source tree `4e07b8441f5815d1f7b34a88dcbf56f498bb0de01a10da108be3ad7d58e8d95a` to the selected binary, 22 receipts, all passed phases, 12 runtime cases, parsed successful-stream headers, and zero owned-container residue. P7c Web remains pending; browser/proxy behavior, global-1000 and sustained load, native systemd and production deployment remain **Not verified**. |
