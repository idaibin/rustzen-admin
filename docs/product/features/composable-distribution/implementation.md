...
| `monitor-notify` | Selected Admin/Controller Cargo, Web, API/schema/config, protocol and native-layout producers; the Admin native config merges access and notifications owners while Monitor retains its notification configuration | Admitted to source/build certification | Exact build, native runtime, installation, browser and load evidence remain Not verified |
