...
| `monitor-notify` | Selected Admin/Monitor Cargo, composition-qualified Web, API/schema/config/protocol/native-layout and exact static container-export producers; Admin native config merges access and notifications owners while Monitor retains its notification configuration | Admitted to source/build/export certification | Real Linux container export, native runtime, installation, browser and load evidence remain Not verified |
