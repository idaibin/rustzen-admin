...
| `analytics` | Explicit Admin+Insights selected Cargo, fresh schema, composition-qualified Web containing only access and Analytics routes/API imports, reviewed access/Insights config, canonical API, delegation protocol and native-layout producers | Admitted to P8a source/build certification | P8b container export/staging, signing, runtime, browser and release certification remain unimplemented; Docker continues to reject Analytics |
