...
- The assignable permission catalog is generated by Admin route registration
  and module Manifests. Operators assign existing capabilities in role
  management but cannot create capability definitions from the menu surface.
- The role list response exposes the current user-assignment count and a
  backend-derived deletion affordance. The assignment count is informational;
  the delete transaction remains the final authority when concurrent changes
  occur.
...
- manually creating capability definitions;
...
- The role list shows the current assignment count. A custom role with a positive
  count has a disabled delete affordance with bilingual accessible guidance,
  while an unused custom role keeps the existing confirmation flow.
- Every new fixed message has Simplified Chinese and English presentation.

## Verification cases

- A real Axum/SQLite list request returns an unused custom role as currently
  deletable and an assigned custom role as currently blocked.
- The same list response keeps built-in roles non-deletable.
- The frontend deletion-state derivation covers both zero and positive counts;
  no assigned role can enter the confirmation flow.
