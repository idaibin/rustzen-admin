...
`/usr/local/bin/rz` command, reloads the daemon, and enables `rz-full.service`
without starting services. It generates local runtime
...
actor. `rz-update.path` starts the fixed root-only `rz-update.service` outside the
Admin service cgroup; the helper rejects links, non-regular files, malformed requests
and invalid release IDs before invoking the worker. The worker:
