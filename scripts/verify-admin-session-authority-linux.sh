...
docker_bin=${RUSTZEN_ADMIN_SESSION_DOCKER:-docker}
source_identity=${RUSTZEN_ADMIN_SESSION_SOURCE_IDENTITY:-"$root/scripts/admin-browser-source-identity.sh"}
verifier_helper=${RUSTZEN_ADMIN_SESSION_VERIFIER_HELPER:-"$root/scripts/ensure-admin-browser-verifier-image.sh"}
file_bin=${RUSTZEN_ADMIN_SESSION_FILE:-file}
timeout=${RUSTZEN_ADMIN_SESSION_TIMEOUT:-900}
cleanup_timeout=${RUSTZEN_ADMIN_SESSION_CLEANUP_TIMEOUT:-10}
kill_grace=${RUSTZEN_ADMIN_SESSION_KILL_GRACE:-5}
helper_timeout=${RUSTZEN_ADMIN_SESSION_HELPER_TIMEOUT:-1020}
helper_cleanup_grace=${RUSTZEN_ADMIN_SESSION_HELPER_CLEANUP_GRACE:-20}
evidence_root=${RUSTZEN_ADMIN_SESSION_EVIDENCE_ROOT:-"$root/target/rz/admin-session-authority"}
...
validate_seconds "$timeout" RUSTZEN_ADMIN_SESSION_TIMEOUT 1800
validate_seconds "$cleanup_timeout" RUSTZEN_ADMIN_SESSION_CLEANUP_TIMEOUT 30
validate_seconds "$kill_grace" RUSTZEN_ADMIN_SESSION_KILL_GRACE 5
validate_seconds "$helper_timeout" RUSTZEN_ADMIN_SESSION_HELPER_TIMEOUT 2100
validate_seconds "$helper_cleanup_grace" RUSTZEN_ADMIN_SESSION_HELPER_CLEANUP_GRACE 30
run_bounded(){ seconds=$1; shift; marker=$(mktemp "${TMPDIR:-/tmp}/rz-admin-session-timeout.XXXXXX"); rm -f "$marker"; "$@" & command_pid=$!; (sleep "$seconds"; : >"$marker"; stop_tree "$command_pid"; sleep "$kill_grace"; kill -KILL "$command_pid" 2>/dev/null || true) & watchdog_pid=$!; if wait "$command_pid"; then status=0; else status=$?; fi; stop_tree "$watchdog_pid"; wait "$watchdog_pid" 2>/dev/null || true; [ ! -e "$marker" ] || status=124; rm -f "$marker"; command_pid=; watchdog_pid=; return "$status"; }
...
run_helper_bounded(){ seconds=$1; shift; marker=$(mktemp "${TMPDIR:-/tmp}/rz-admin-session-helper-timeout.XXXXXX"); rm -f "$marker"; helper_active=1; "$@" & command_pid=$!; (sleep "$seconds"; : >"$marker"; stop_helper "$command_pid") & watchdog_pid=$!; if wait "$command_pid"; then status=0; else status=$?; fi; helper_active=0; stop_tree "$watchdog_pid"; wait "$watchdog_pid" 2>/dev/null || true; [ ! -e "$marker" ] || status=124; rm -f "$marker"; command_pid=; watchdog_pid=; return "$status"; }
run_capture(){ capture=$(mktemp "${TMPDIR:-/tmp}/rz-admin-session-command.XXXXXX"); if run_bounded "$@" >"$capture"; then status=0; else status=$?; fi; cat "$capture"; rm -f "$capture"; return "$status"; }
...
build_container=rz-admin-session-build-$run_id; runtime_container=rz-admin-session-runtime-$run_id
mkdir "$lock" 2>/dev/null || { echo 'another Admin session gate owns the lock' >&2; exit 1; }; mkdir "$candidate" "$staged"
...
build_active=1
if run_bounded "$timeout" "$docker_bin" run --name "$build_container" --platform "$platform" --mount "type=bind,src=$root,dst=/work" --mount type=volume,src=rustzen-admin-session-cargo,dst=/usr/local/cargo/registry -w /work rust:1.95-bookworm bash -euo pipefail -c "
 ...
 export RUSTFLAGS='-C target-feature=+crt-static'; target=target/admin-session-authority/cargo; out=target/rz/admin-session-authority/build/$architecture/bin
...
if run_bounded "$timeout" "$docker_bin" run --name "$runtime_container" --platform "$platform" --security-opt seccomp=unconfined \
 ...
 --mount "type=bind,src=$staged,dst=/verify/bin,readonly" --mount "type=bind,src=$candidate,dst=/verify/evidence" --mount "type=bind,src=$root/scripts/verify-admin-session-authority-linux-inner.sh,dst=/verify/run.sh,readonly" "$verifier_image" bash /verify/run.sh >"$candidate/runtime.log" 2>&1; then runtime_status=0; else runtime_status=$?; fi
...
allowlist=$(jq -nc '["steps.log","sid-a-after-logout.json","sid-b-after-logout.json","revoke-all.json","password-old.json","password-new.json","disable-old.json","disable-login.json","grant-before.json","grant-removed.json","grant-restored.json","bad-jwt.json","valid-before-expiry.json","expired-jwt.json","authority-db-failure.json","selected-state.json","phase-status.tsv","build.log","verifier-helper.log","runtime.log"]')
jq -e --arg head "$head" --arg state "$source_state" --arg sourceSha "$source_sha" --arg architecture "$architecture" --arg platform "$platform" --arg binary "$binary_sha" --arg provenance "$provenance_sha" --arg verifierImage "$verifier_image" --arg verifierKey "$verifier_key" --arg verifierSha "$verifier_sha" --argjson allowlist "$allowlist" \
 --slurpfile sidA "$candidate/sid-a-after-logout.json" --slurpfile sidB "$candidate/sid-b-after-logout.json" --slurpfile revokeAll "$candidate/revoke-all.json" --slurpfile passwordOld "$candidate/password-old.json" --slurpfile passwordNew "$candidate/password-new.json" --slurpfile disableOld "$candidate/disable-old.json" --slurpfile disableLogin "$candidate/disable-login.json" --slurpfile grantBefore "$candidate/grant-before.json" --slurpfile grantRemoved "$candidate/grant-removed.json" --slurpfile grantRestored "$candidate/grant-restored.json" --slurpfile badJwt "$candidate/bad-jwt.json" --slurpfile validBeforeExpiry "$candidate/valid-before-expiry.json" --slurpfile expiredJwt "$candidate/expired-jwt.json" --slurpfile authorityFailure "$candidate/authority-db-failure.json" --slurpfile selected "$candidate/selected-state.json" --rawfile phaseStatus "$candidate/phase-status.tsv" --rawfile buildLog "$candidate/build.log" --rawfile helperLog "$candidate/verifier-helper.log" --rawfile runtimeLog "$candidate/runtime.log" -f "$root/scripts/verify-admin-session-authority-evidence.jq" "$candidate/manifest.json" >/dev/null
...
read -r final_head final_state final_sha < <("$source_identity"); [ "$final_head:$final_state:$final_sha" = "$head:$source_state:$source_sha" ] || { echo 'source tree changed during Admin session gate' >&2; exit 1; }
mv "$candidate" "$published"; ln -s "runs/$run_id" "$publish_link"; atomic_replace_symlink "$publish_link" "$current"
echo "Admin session authority evidence published: $current/manifest.json"
