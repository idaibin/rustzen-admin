...
test("source-build certificate CLI publishes one canonical no-replace certificate", async () => {
    ...
    try {
        ...
        expect(Bun.spawnSync(command, { cwd: repository }).exitCode).not.toBe(
            0,
        );
        for (const args of [
            [],
...
test("source-build certificate CLI publishes monitor-notify only for its signed release", async () => {
    const repository = resolve(import.meta.dir, "..");
    await mkdir(join(repository, "target"), { recursive: true });
    const root = await mkdtemp(
        join(repository, "target", "source-build-notify-cli-"),
    );
    const notify = {
        schemaVersion: 1,
    ...
    const fixtureExport = await createExport(notify);
    const exportRoot = join(root, "export");
    const keys = generateKeyPairSync("ed25519");
    const privateKey = keys.privateKey
        .export({ type: "pkcs8", format: "pem" })
    ...
    const publicKey = keys.publicKey
        .export({ type: "spki", format: "pem" })
        .toString();
    const trusted = { keyId: "source-build-notify-cli", publicKey };
    try {
        await cp(fixtureExport, exportRoot, { recursive: true });
        await writeFile(join(root, "public.pem"), publicKey, { mode: 0o644 });
        const snapshot = await verifyContainerExport(
        ...
        const relative = (path: string) => path.slice(repository.length + 1);
        const command = [
            process.execPath,
            "scripts/distribution-issue-source-build-certificate.ts",
            "--selection",
        ...
        const certificate = parseSourceBuildCertificate(
            await Bun.file(output.path).json(),
            notify,
        );
        expect(certificate.selection).toMatchObject({
            preset: "monitor-notify",
            compositionId: snapshot.manifest().compositionId,
        });
        expect(certificate.certifiedLayers.build.buildId).toBe(release.buildId);
        expect([
            certificate.runtime,
            certificate.browser,
            certificate.load,
            certificate.releaseReady,
