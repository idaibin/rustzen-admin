...
remove_container() {
  run_bounded "$cleanup_timeout" "$docker_bin" rm -f "$container" >/dev/null 2>&1 || { echo "failed to remove Docker container $container" >&2; return 1; }
  listing=$(run_bounded_capture "$cleanup_timeout" "$docker_bin" ps -a --filter "name=^/$container$" --format '{{.Names}}') || { echo "Docker absence check failed for $container" >&2; return 1; }
  [ -z "$listing" ] || { echo "Docker container still exists after removal: $container" >&2; return 1; }
}
preserve_failure() {
  mkdir -p "$failed"
  printf 'exitStatus\t%s\ncleanupStatus\t%s\ncontainer\t%s\n' "$1" "$2" "$container" >"$failed/failure-summary.tsv"
  for file in container.log build-provenance.txt rz-monitor-agent.provenance.tsv; do [ ! -f "$candidate/$file" ] || cp "$candidate/$file" "$failed/$file"; done
}
...
cleanup() {
  ...
  if [ "$cleanup_status" -eq 0 ]; then rm -rf "$lock_dir"; else echo "cleanup failed; ownership lock retained at $lock_dir" >&2; status=$cleanup_status; fi
  exit "$status"
}
...
if [ "${RUSTZEN_MONITOR_MULTI_NODE_UI_TEST_CLEANUP:-}" = 1 ]; then
  evidence_root=${RUSTZEN_MONITOR_MULTI_NODE_UI_TEST_ROOT:?missing test root}; current="$evidence_root/current"; lock_dir="$evidence_root/.verify.lock"; run_id=test; candidate="$evidence_root/.candidate"; staged="$evidence_root/.binaries"; failed="$evidence_root/failed-runs/$run_id"; container=stubborn; container_active=1; mkdir -p "$evidence_root/runs/old" "$candidate" "$staged" "$lock_dir"; printf safe >"$candidate/build-provenance.txt"; printf rustzen@123 >"$candidate/browser-steps.json"; ln -s runs/old "$current"; trap cleanup EXIT; exit 1
fi
...
mkdir -p "$evidence_root/runs" "$evidence_root/failed-runs"; if ! mkdir "$lock_dir" 2>/dev/null; then echo "another dual-Agent Nodes verification owns $lock_dir" >&2; exit 1; fi
run_id="$(date -u +%Y%m%dT%H%M%SZ)-$$"; candidate="$evidence_root/.candidate-$run_id"; staged="$evidence_root/.binaries-$run_id"; failed="$evidence_root/failed-runs/$run_id"; container="rz-monitor-agent-multi-node-ui-$run_id"; status=1
