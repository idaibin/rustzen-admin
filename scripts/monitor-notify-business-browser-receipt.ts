...
const epochMilliseconds = (value: unknown): value is number => typeof value === "number" && Number.isSafeInteger(value) && value >= 1_000_000_000_000;
...
function validJourney(value: any, provenance: unknown) {
    ...
    return exact(value, ["api", "ids", "routes", "screenshots", "sse", "ui", "witnessAgent"])
        ...
        && exact(sse, ["badgeObservedAt", "bearer", "contentType", "frameBytes", "frameBytesBefore", "listFetchTimestamp", "method", "readyTimestamp", "reportIssuedAt", "status", "tokenInUrl", "url"]) && sse.bearer && !sse.tokenInUrl && sse.method === "GET" && sse.url === "/api/notifications/stream" && sse.status === 200 && String(sse.contentType).includes("text/event-stream") && epochMilliseconds(sse.readyTimestamp) && epochMilliseconds(sse.reportIssuedAt) && epochMilliseconds(sse.listFetchTimestamp) && epochMilliseconds(sse.badgeObservedAt) && sse.frameBytes > sse.frameBytesBefore && sse.readyTimestamp < sse.reportIssuedAt && sse.reportIssuedAt < sse.listFetchTimestamp && sse.listFetchTimestamp < sse.badgeObservedAt
        ...
        && exact(api.inbox, ["itemCount", "listFetchTimestamp", "revision", "snapshot", "status"]) && api.inbox.status === 200 && Number.isSafeInteger(api.inbox.itemCount) && api.inbox.itemCount > 0 && Number.isSafeInteger(api.inbox.revision) && api.inbox.revision >= 0 && text(api.inbox.snapshot) && api.inbox.listFetchTimestamp === sse.listFetchTimestamp
