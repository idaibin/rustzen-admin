...
function validJourney(value: any, provenance: unknown) {
    ...
    return exact(value, ["api", "ids", "routes", "screenshots", "sse", "ui", "witnessAgent"])
        ...
        && exact(sse, ["bearer", "contentType", "frameBytes", "frameBytesBefore", "listFetchTimestamp", "method", "readyTimestamp", "reportIssuedAt", "status", "tokenInUrl", "url"]) && sse.bearer && !sse.tokenInUrl && sse.method === "GET" && sse.url === "/api/notifications/stream" && sse.status === 200 && String(sse.contentType).includes("text/event-stream") && Number.isFinite(sse.readyTimestamp) && Number.isFinite(sse.reportIssuedAt) && Number.isFinite(sse.listFetchTimestamp) && sse.frameBytes > sse.frameBytesBefore && sse.listFetchTimestamp >= sse.reportIssuedAt
        ...
        && exact(api.inbox, ["itemCount", "listFetchTimestamp", "revision", "snapshot", "status"]) && api.inbox.status === 200 && Number.isSafeInteger(api.inbox.itemCount) && api.inbox.itemCount > 0 && Number.isSafeInteger(api.inbox.revision) && api.inbox.revision >= 0 && text(api.inbox.snapshot) && Number.isFinite(api.inbox.listFetchTimestamp)
