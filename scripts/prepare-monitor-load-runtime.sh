...
usage() { echo "usage: $0 --export-root PATH --release-result FILE --certificate FILE --public-key FILE --expected-source-identity ID --native-output NEW_DIRECTORY --browser-output NEW_DIRECTORY --context-output NEW_FILE [--chromium PATH]" >&2; exit 2; }
test "$#" -eq 16 -o "$#" -eq 18 || usage
chromium=chromium
