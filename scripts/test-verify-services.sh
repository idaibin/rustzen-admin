...
grep -Fqx 'verify_module_log_diagnostics() {' "$VERIFY"
grep -Fqx '    wait_for_status 403 "http://127.0.0.1:$RUSTZEN_ADMIN_PORT/api/system/status/module-logs?module=admin&date=$module_log_date" "$denied_token"' "$VERIFY"
grep -Fqx '        const expectedLines = 24_000;' "$VERIFY"
grep -Fqx '            if (!tail.truncated || tail.nextCursor === cursor) {' "$VERIFY"
grep -Fqx '        if (reconstructed.length !== expectedLines || reconstructed.some((id, index) => id !== index)) {' "$VERIFY"
grep -Fqx '        if (!/^attachment;\s*filename=rustzen-module-logs\.tar$/i.test(value("content-disposition") ?? "")) throw new Error("missing backup filename header");' "$VERIFY"
grep -Fqx '        if (value("x-rustzen-archive-file-count") !== "1") throw new Error("backup count header did not match one selected file");' "$VERIFY"
grep -Fqx '    [ "$repeated_status" = 400 ] || { echo "verify-services: reused module-log cleanup token returned $repeated_status" >&2; exit 1; }' "$VERIFY"
