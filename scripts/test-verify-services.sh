...
AUTH_MODULE_GATEWAY_HELPER="${VERIFY_SERVICES_AUTH_MODULE_GATEWAY_HELPER:-$ROOT/scripts/verify-service-auth-module-gateway.sh}"

bash -n "$VERIFY"
sh -n "$MODULE_LOG_HELPER"
sh -n "$DATABASE_ISOLATION_HELPER"
sh -n "$LIFECYCLE_HELPER"
sh -n "$AUTH_MODULE_GATEWAY_HELPER"
...
grep -Fqx 'AUTH_MODULE_GATEWAY_HELPER="$PROJECT_ROOT/scripts/verify-service-auth-module-gateway.sh"' "$VERIFY"
grep -Fqx 'if [ ! -f "$AUTH_MODULE_GATEWAY_HELPER" ] || [ -L "$AUTH_MODULE_GATEWAY_HELPER" ]; then' "$VERIFY"
grep -Fqx 'if ! . "$AUTH_MODULE_GATEWAY_HELPER"; then' "$VERIFY"
if grep -Fq 'wait_for_module_state() {' "$VERIFY"; then
    echo "verify-services must source auth/module-gateway helpers instead of defining them inline" >&2
    exit 1
fi
grep -Fqx 'parse_json() {' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fqx 'login() {' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fqx 'wait_for_module_state() {' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fqx '    while [ "$count" -lt 180 ]; do' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fq '/api/monitor/nodes' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fq '/api/insights/overview' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fq '/api/reports/systems' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fqx '        if (payload.code !== 40001 || payload.message !== expected || payload.data !== null) {' "$AUTH_MODULE_GATEWAY_HELPER"
grep -Fqx '    for module in monitor insights reports; do' "$AUTH_MODULE_GATEWAY_HELPER"
