...
import { createExport, sourceIdentity } from "../distribution/container-export-test-fixture.ts";
...
test("captured Monitor release CLI strictly signs a verified export without printing PEMs", async () => {
    ...
    try {
        ...
        await writeFile(join(root, "private.pem"), privateKey, { mode: 0o600 });
        await writeFile(join(root, "public.pem"), keys.publicKey.export({ type: "spki", format: "pem" }), { mode: 0o644 });
        const command = [
            process.execPath, "scripts/distribution-publish-monitor-container-release.ts",
            "--selection", "distribution/fixtures/monitor.json",
            "--export-root", "target/container-release-cli-test/export",
            "--expected-source-identity", sourceIdentity,
            "--output-base", "target/container-release-cli-test/output",
            "--private-key", join(root, "private.pem"),
            "--public-key", join(root, "public.pem"), "--key-id", "captured-cli",
        ];
        const invalidPublic = command.map((value, index) =>
            command[index - 1] === "--public-key" ? join(root, "private.pem") : value,
        );
        ...
        expect(await Bun.file(join(root, "invalid-output", ".native-staging")).exists()).toBeFalse();
        ...
        if (found.exitCode !== 0) throw new Error(new TextDecoder().decode(found.stderr));
        const names = new TextDecoder().decode(found.stdout)
            .trim().split("\n").map((path) => basename(path)).sort();
        expect(names).toEqual([
            "archive.tar", "release-manifest.json", "signature-envelope.json",
        ]);
        for (const name of names)
            expect((await stat(join(published.root, name))).mode & 0o777).toBe(0o644);
        const trusted = {
            keyId: "captured-cli",
        ...
        await expect(verifyReleaseSnapshot(
            published.root,
            { preset: "monitor", target: "x86_64-unknown-linux-musl" },
            trusted,
        )).rejects.toThrow();
        const duplicate = Bun.spawnSync(command, { cwd: repository, stdout: "pipe", stderr: "pipe" });
        expect(duplicate.exitCode).not.toBe(0);
        for (const args of [[], ["--unknown", "x"], ["--selection", "distribution/fixtures/monitor.json", "--selection", "distribution/fixtures/monitor.json"]]) {
            const invalid = Bun.spawnSync([process.execPath, "scripts/distribution-publish-monitor-container-release.ts", ...args], { cwd: repository, stdout: "pipe", stderr: "pipe" });
