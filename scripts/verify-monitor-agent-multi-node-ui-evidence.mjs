...
const canonical = (nodes) => {
    ...
    const detail = ({ nodeId, bootId }) => [{ action: "waitFor", selector: `[data-testid="monitor-node-view"][data-node-id="${nodeId}"]` }, { action: "assertText", selector: selector("boot-id", nodeId), text: bootId }, { action: "click", selector: `[data-testid="monitor-node-view"][data-node-id="${nodeId}"]` }, { action: "waitFor", selector: "[data-testid=monitor-node-details]" }, { action: "waitFor", selector: selector("details-boot-id", nodeId) }, { action: "assertText", selector: selector("details-boot-id", nodeId), text: bootId }, { action: "waitFor", selector: selector("history-5m", nodeId) }, { action: "assertText", selector: selector("history-5m", nodeId), text: "5-minute history" }, { action: "waitFor", selector: `${selector("history-5m", nodeId)} .recharts-line-dot` }, { action: "screenshotViewport", name: `monitor-agent-${nodeId}-detail-dark-en` }, { action: "pressKey", key: "Escape" }];
