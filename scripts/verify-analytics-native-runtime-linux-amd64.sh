...
selection_preset="$(pnpm dlx bun@1.3.14 -e 'const value=await Bun.file(process.argv[1]).json(); if(value?.preset!=="analytics") throw Error("runtime selection is unsupported"); console.log(value.preset)' "$selection")"
...
admission="$(pnpm dlx bun@1.3.14 scripts/distribution-verify-published-source-build-certificate.ts \
  ...
  --public-key "$public_key" --key-id "$key_id" --certificate "$certificate" --evidence linux-amd64-buildkit)"
mkdir "$output"
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && docker ps -a --format '{{.Names}}' | grep -Fxq "$name"; then
    docker exec "$name" systemctl --no-pager --full status rz-admin.service rz-insights.service >> "$output/runtime.log" 2>&1 || true
    docker exec "$name" journalctl --no-pager -u rz-admin.service -u rz-insights.service >> "$output/runtime.log" 2>&1 || true
...
EOF
  ...
  if ! "$rz" --json activate-analytics-server --config /root/rz-activation/server.env > /root/rz-activation/activate.json; then cat /root/rz-activation/activate.json >&2; exit 1; fi
  ...
  systemctl is-enabled --quiet rz-full.service
  systemctl is-active --quiet rz-full.service
  for unit in rz-admin.service rz-insights.service; do systemctl is-active --quiet "$unit"; done
  admin_health=$(curl --fail --silent http://127.0.0.1:19801/health); insights_health=$(curl --fail --silent http://127.0.0.1:19802/health)
  printf "%s" "$admin_health" | grep -Fq "$binding"; printf "%s" "$insights_health" | grep -Fq "$binding"
  ...
  for absent in /etc/systemd/system/rz-monitor.service /etc/systemd/system/rz-reports.service /opt/rz/config/rz-monitor.env /opt/rz/config/rz-reports.env /var/lib/rustzen-monitor /var/lib/rustzen-reports; do test ! -e "$absent"; done
  systemctl restart rz-admin.service rz-insights.service
  wait_health
  systemctl stop rz-admin.service rz-insights.service; systemctl start rz-admin.service; systemctl start rz-insights.service
  wait_health
  systemctl is-active --quiet rz-admin.service; systemctl is-active --quiet rz-insights.service
  systemctl stop rz-admin.service rz-insights.service; systemctl start rz-insights.service; systemctl start rz-admin.service
  wait_health
  systemctl is-active --quiet rz-admin.service; systemctl is-active --quiet rz-insights.service
  admin_pid=$(systemctl show -p MainPID --value rz-admin.service); insights_pid=$(systemctl show -p MainPID --value rz-insights.service)
  for pair in "rz-admin.service:$admin_pid:rz-admin" "rz-insights.service:$insights_pid:rz-insights"; do
...
if test -n "${retain_output:-}"; then
  ...
  pnpm dlx bun@1.3.14 -e 'import { canonicalJson } from "./distribution/release-manifest-core.ts"; const [out,name,id,image,port,evidence,ownerToken]=process.argv.slice(1); await Bun.write(out,canonicalJson({containerId:id,containerName:name,containerPort:19801,hostPort:Number(port),imageId:image,nativeEvidence:evidence,ownerToken}));' "$retain_output" "$name" "$(docker inspect --format '{{.Id}}' "$name")" "$(docker inspect --format '{{.Image}}' "$name")" "$host_port" "$output/analytics-native-runtime-evidence.json" "$retain_owner"
fi
echo "P8e exact-artifact Analytics Linux/amd64 PID1 runtime gate PASS: $output/analytics-native-runtime-evidence.json"
