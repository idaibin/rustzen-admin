...
const hash = "a".repeat(64), composition = "8957924886140f55fd0560d89f0c2acdac67cd95d14c09ac78d6f9fa18109d3b", fixtureSourceIdentity = `git:${"b".repeat(40)} tree:${"c".repeat(64)} state:clean`, alternateSourceIdentity = `git:${"d".repeat(40)} tree:${"c".repeat(64)} state:clean`, health = { status: "ok", selectedBinding: { buildId: hash, compositionId: composition } }, runtime = { containerId: "container", containerPort: 8080, dev: "1", hostPort: 3000, imageId: "image", ino: "2", pid: 3, sha256: hash }, native = {
...
test("P8e/P8f admission rejects unknown fields and noncanonical files", async () => {
    ...
    const notify = notifyTuple(); expect(parseSelectedWebBootstrapBrowserReceipt(notify.browser)).toBe(notify.browser); expect(admitted(notify.native, notify.browser)).toMatchObject({ selection: notify.native.selection });
    const independent = structuredClone(browser); independent.sourceIdentity.verifierSourceIdentity = alternateSourceIdentity; expect(parseSelectedWebBootstrapBrowserReceipt(independent)).toBe(independent); expect(admitted(native, independent)).toMatchObject({ selection: native.selection });
    const cleared = structuredClone(browser); cleared.sourceIdentity.verifierSourceIdentity = ""; expect(() => admitted(native, cleared)).toThrow();
    ...
    expect(() => admitted(native, { ...browser, sourceIdentity: { productSourceIdentity: fixtureSourceIdentity } })).toThrow();
    expect(() => admitted(native, { ...browser, sourceIdentity: { ...browser.sourceIdentity, extra: true } })).toThrow();
...
test("shared P8f parser admits the real receipt and rejects every protected section mutation", async () => {
    const real = JSON.parse(JSON.stringify({ ...browser, verifier: { sources: [{ path: "scripts/fixture.py", sha256: hash }] } }));
    ...
    for (const mutate of [value => { value.cases[0].browser.entryExecuted = false; }, value => { value.cases[1].browser.reloadCount = 0; }, value => { value.cases[2].requests[0].authorization = true; }, value => { value.cases[3].assertions.failureApiAbsent = false; }, value => { value.sourceIdentity.productSourceIdentity = "changed"; }, value => { value.adminHealth.final.status = "bad"; }]) {
        const changed = structuredClone(real); mutate(changed); expect(() => parseSelectedWebBootstrapBrowserReceipt(changed)).toThrow();
    }
    for (const mutate of [value => { value.cases[0].browser.value = null; }, value => { value.cases[0].browser.stamp = "b".repeat(64); }, value => { value.cases[0].browser.url = "http://example.invalid/login"; }, value => { value.sensitivity.marker = false; }, value => { value.sensitivity.requests.find(x => x.path === "/__web-binding").cookie = true; }, value => { value.cases[0].browser.value.login = false; }, value => { value.cases[0].browser.value.extra = true; }, value => { value.cases[0].health.before.selectedBinding.buildId = "b".repeat(64); }]) { const changed=structuredClone(real); mutate(changed); expect(() => parseSelectedWebBootstrapBrowserReceipt(changed)).toThrow(); }
...
test("phase receipt parses, publishes canonically, and rejects phase binding mutations", async () => {
    ...
    const notifyReceipt = structuredClone(receipt);
    notifyReceipt.selection.preset = "monitor-notify";
    notifyReceipt.sseBoundary = { runtimeStatus: 200, contentType: "text/event-stream", signedPresence: { apiOwners: 3, schemaOwners: 4, notificationWebRoutes: 1 } };
    notifyReceipt.p8eSidecars["/ingress"] = hash;
    expect(parseMonitorLoadReceipt(notifyReceipt)).toBe(notifyReceipt);
    expect(() => parseMonitorLoadReceipt({ ...notifyReceipt, sseBoundary: { runtimeStatus: 200, contentType: "application/json", signedPresence: {} } })).toThrow("sse boundary differs");
    expect(() => parseMonitorLoadReceipt({ ...notifyReceipt, p8eSidecars: Object.fromEntries(Object.entries(notifyReceipt.p8eSidecars).slice(0, 12)) })).toThrow("P8e sidecars");
