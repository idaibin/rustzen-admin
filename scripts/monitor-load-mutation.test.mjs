...
test("P8e/P8f admission rejects unknown fields and noncanonical files", async () => {
    ...
    const independent = structuredClone(browser); independent.sourceIdentity.verifierSourceIdentity = alternateSourceIdentity; expect(parseSelectedWebBootstrapBrowserReceipt(independent)).toBe(independent); expect(() => admitted(native, independent)).toThrow("tuple differs");
    ...
    expect(() => admitted(native, { ...browser, sourceIdentity: { ...browser.sourceIdentity, verifierSourceIdentity: alternateSourceIdentity } })).toThrow("tuple differs");
