...
cleanup(){ status=$?; test -z "$chrome_pid" || kill "$chrome_pid" 2>/dev/null || true; rm -rf "$profile"; test "$status" -eq 0 || rm -rf "$candidate"; exit "$status"; }; trap cleanup EXIT INT TERM
...
rm -rf "$profile"; rm -f "$log"; verify_output "$candidate"; mv "$candidate" "$output"; verify_output "$output"
