...
schedule_columns="$root/apps/web/src/routes/reports/-templates/schedule-columns.tsx"
...
for file in "$outer" "$inner" "$docs" "$session" "$identity" "$provenance" "$build" "$manifest_verifier" "$ensure" "$verifier_dockerfile" "$schedule_panel" "$schedule_columns" "$schedule_utils" "$schedule_utils_test"; do test -s "$file"; done
...
grep -Fq 'data-due-local={occurrence.dueLocal}' "$schedule_columns"
...
grep -Fq 'refresh_api_auth' "$inner"
grep -Fq 'RUSTZEN_UI_BROWSER_RUN_TIMEOUT:-2400' "$outer"
grep -Fq 'must be 1..2400 seconds' "$outer"
