...
const script = await Bun.file(
    resolve(import.meta.dir, "verify-analytics-native-runtime-linux-amd64.sh"),
).text();
...
test("just recipe forwards the explicit runtime tuple", () => {
    ...
    expect(justfile).toContain("distribution/monitor-native-runtime-revalidator.test.ts");
    expect(justfile).toContain("scripts/verify-analytics-native-runtime-linux-amd64.test.ts");
});
...
test("P8e gate requires one explicit current tuple and fresh output", () => {
    for (const value of [
        ...
        "rz-insights.service",
        "analytics-native-runtime-evidence.json",
        "/etc/systemd/system/rz-monitor.service",
        "/var/lib/rustzen-monitor",
    ])
...
test("P8e gate proves the required positive and negative runtime checks", () => {
    for (const value of [
        ...
        "systemctl start rz-insights.service",
    ])
        expect(script).toContain(value);
});
