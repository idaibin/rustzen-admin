...
const output = resolve(
    repository,
    process.env.RUSTZEN_INSTALLER_OUTPUT ?? "target/installer-fixture",
);
...
const webMutation = process.env.RUSTZEN_INSTALLER_WEB_MUTATION;
const artifact = process.env.RUSTZEN_INSTALLER_ARTIFACT ?? "server";
const agentBinary = process.env.RUSTZEN_INSTALLER_AGENT_BINARY;
const serverAdminBinary = process.env.RUSTZEN_INSTALLER_ADMIN_BINARY;
const serverMonitorBinary = process.env.RUSTZEN_INSTALLER_MONITOR_BINARY;
if (Boolean(serverAdminBinary) !== Boolean(serverMonitorBinary)) {
    throw new Error("server fixture requires both Admin and Monitor binaries");
}
await rm(output, { recursive: true, force: true });
await mkdir(output, { recursive: true, mode: 0o700 });
const selection =
    artifact === "node-agent" ? { preset: "node-agent", target } : { preset: "monitor", target };
const fixture =
    artifact === "node-agent"
        ? await agentManifestFixture(selection, agentBinary)
        : await serverManifestFixture(
              selection,
              serverAdminBinary && serverMonitorBinary
...
try {
    ...
    let archive = new Uint8Array(
        await Bun.file(join(publication.root, "archive.tar")).arrayBuffer(),
    );
    ...
    if (webMutation) {
        const relative =
            webMutation === "binding"
                ...
                : webMutation === "index-stamp"
                  ? "web/index.html"
                  : webMutation === "web-file"
                    ? "web/rustzen.png"
                    : (() => {
                          throw new Error(`unknown installer Web mutation: ${webMutation}`);
                      })();
        const path = join(fixture.staging.root, relative);
        const original = new Uint8Array(await Bun.file(path).arrayBuffer());
        const changed = mutateWeb(original, webMutation);
        archive = replace(archive, original, changed);
        manifest.files.find((entry: any) => entry.path === relative).sha256 = sha256(changed);
    }
    if (manifestMutation) mutateManifest(manifest, manifestMutation);
    const manifestBytes = new TextEncoder().encode(canonicalJson(manifest));
    if (manifestMutation || schemaMutation || webMutation)
        archive = replace(archive, originalManifest, manifestBytes);
...
function mutateManifest(manifest: any, kind: string) {
    ...
    if (kind === "capabilities")
        manifest.capabilities[manifest.capabilities.length - 1] = "accessx";
    else if (kind === "config-owners")
        manifest.configOwners[manifest.configOwners.length - 1] = "accessx";
...
function mutateSchema(bytes: Uint8Array, kind: string): Uint8Array {
    ...
    if (!replacement || replacement === text)
        throw new Error(`unknown installer schema mutation: ${kind}`);
    return new TextEncoder().encode(replacement);
}
function mutateWeb(bytes: Uint8Array, kind: string): Uint8Array {
    if (kind === "web-file") {
        const output = bytes.slice();
        output[0] = output[0] === 120 ? 121 : 120;
        return output;
    }
    const text = new TextDecoder().decode(bytes);
    const changed =
        kind === "binding"
            ? text.replace(
                  /"webDigest":"([a-f0-9])/,
                  (_, first) => `"webDigest":"${first === "f" ? "e" : "f"}`,
              )
            : text.replace(
                  /content="([a-f0-9])/,
                  (_, first) => `content="${first === "f" ? "e" : "f"}`,
              );
    if (changed === text) throw new Error(`installer Web mutation failed: ${kind}`);
    return new TextEncoder().encode(changed);
}
function replace(value: Uint8Array, from: Uint8Array, to: Uint8Array): Uint8Array {
    if (from.length !== to.length)
        throw new Error("manifest mutation must preserve archive member size");
    const at = value.findIndex((_, offset) =>
        from.every((byte, index) => value[offset + index] === byte),
    );
