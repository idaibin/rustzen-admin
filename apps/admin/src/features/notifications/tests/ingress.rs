...
async fn signed_ingress_recomputes_recipients_and_deduplicates() {
    ...
    let signed = signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "nonce-1").unwrap();
    ...
    let duplicate =
        signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "nonce-2").unwrap();
    ...
    let conflict = signer.sign(&changed, now.timestamp(), now.timestamp() + 60, "nonce-3").unwrap();
    ...
    let signed = previous.sign(&payload, now.timestamp(), now.timestamp() + 60, "nonce-4").unwrap();
...
async fn ingress_rejects_tamper_replay_expiry_and_oversize() {
    ...
    let signed =
        signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "nonce-auth").unwrap();
    ...
    let expired =
        signer.sign(&payload, now.timestamp() - 60, now.timestamp(), "nonce-expired").unwrap();
    assert_eq!(state.ingest(expired, &payload, now).await, Err(IngestError::Expired));
    let oversized = vec![b'x'; 16 * 1024 + 1];
    let signed =
        signer.sign(&oversized, now.timestamp(), now.timestamp() + 60, "nonce-large").unwrap();
    ...
    let signed = previous
        .sign(&payload, observed.timestamp(), observed.timestamp() + 60, "nonce-old-key")
        .unwrap();
...
async fn dedicated_http_route_enforces_protocol_headers_and_content_type() {
    ...
    let signed =
        signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "nonce-http-valid").unwrap();
