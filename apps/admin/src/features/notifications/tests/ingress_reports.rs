...
async fn reports_key_and_current_authority_are_both_producer_scoped() {
    ...
    let wrong = NotificationSigner::new("shared-id", "reports", MONITOR_KEY)
        .unwrap()
        .sign(&payload, now.timestamp(), now.timestamp() + 60, "wrong-key")
        .unwrap();
    ...
    let signed = signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "stored").unwrap();
    ...
    let duplicate =
        signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "duplicate").unwrap();
    ...
    let signed =
        signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "disabled-user").unwrap();
    ...
    let signed =
        signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "deleted-user").unwrap();
    assert_eq!(state.ingest(signed, &payload, now).await, Ok(IngestOutcome::NoRecipients));
    ...
    let signed = signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "revoked").unwrap();
    assert_eq!(state.ingest(signed, &payload, now).await, Ok(IngestOutcome::NoRecipients));
    ...
    let signed = signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "disabled").unwrap();
    assert_eq!(state.ingest(signed, &payload, now).await, Ok(IngestOutcome::NoRecipients));
...
async fn reports_rejects_cross_topic_and_missing_initiator() {
    ...
    let signed = signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "cross").unwrap();
    ...
    let payload = serde_json::to_vec(&event).unwrap();
    let signed = signer.sign(&payload, now.timestamp(), now.timestamp() + 60, "missing").unwrap();
    assert_eq!(state.ingest(signed, &payload, now).await, Err(IngestError::Forbidden));
    database.close().await;
}
