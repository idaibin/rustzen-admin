...
impl DeployRepository {
    ...
    pub async fn has_current(&self) -> Result<bool, ServiceError> {
        let count = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM deploy_versions WHERE is_current = 1 AND deleted_at IS NULL",
        )
        .fetch_one(&self.pool)
        .await
        .map_err(map_db_error)?;
    ...
    pub async fn upsert_installed_current(
        &self,
        payload: &DeploymentPayload,
    ) -> Result<(), ServiceError> {
        let mut tx = self.pool.begin().await.map_err(map_db_error)?;
        sqlx::query(
            "UPDATE deploy_versions SET is_current = 0, updated_at = CURRENT_TIMESTAMP WHERE is_current = 1",
        )
        .execute(&mut *tx)
        .await
        .map_err(map_db_error)?;
        sqlx::query(
            ...
            ON CONFLICT(component, version, arch) DO UPDATE SET
                ...
                expired_at = NULL,
                deleted_at = NULL,
                deployed_at = COALESCE(deploy_versions.deployed_at, CURRENT_TIMESTAMP),
    ...
    pub async fn set_current(
        ...
        if result.rows_affected() == 0 {
            return Err(ServiceError::InvalidOperation(
                "Release is expired, already current, or changed while the update was running"
                    .to_string(),
            ));
        }
    ...
    pub async fn delete_by_id(&self, version_id: i64) -> Result<DeploymentItem, ServiceError> {
        ...
        if result.rows_affected() == 0 {
            self.ensure_not_current(version_id, "delete").await?;
        }
