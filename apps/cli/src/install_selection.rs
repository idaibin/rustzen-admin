...
pub(super) fn digest(preset: &str, target: &str) -> Result<String, String> {
    ...
    let text = match preset {
        ...
        // Resolver-derived selection digests for the reviewed Analytics closure.
        "analytics" => {
            return match target {
                "x86_64-unknown-linux-musl" => {
                    Ok("799d104faafc7c0a04ae45211461fec03d159208f08194d181fdd02e0e0eedbb".into())
                }
                "aarch64-unknown-linux-gnu" => {
                    Ok("5beed2b0836aa80b90d4b7f059ab41fc8866c68a49069dbc32446ed30996345c".into())
                }
                _ => Err("selection target is unsupported".into()),
            };
        }
