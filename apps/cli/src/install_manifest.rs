...
pub(super) fn parse_manifest(bytes: &[u8]) -> Result<Manifest, String> {
    ...
    let expected = base.into_iter().collect();
...
fn validate(m: &Manifest) -> Result<(), String> {
    ...
    if m.artifact_class != "node-agent" || !agent(m, &files)? {
...
fn agent(m: &Manifest, files: &BTreeMap<&str, &Entry>) -> Result<bool, String> {
    ...
    {
        return Err("node-agent manifest selection is invalid".into());
    }
...
fn exact_paths(files: &BTreeMap<&str, &Entry>, fixed: &[&str]) -> bool {
    files.keys().all(|path| fixed.contains(path))
