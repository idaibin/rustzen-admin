...
async fn injected_storage_capacity_rejection_preserves_existing_rows() {
    ...
    let app = build_router_with_storage_capacity_checker(pool, SECRET, checker).expect("router");
...
async fn collection_policy_put_rotates_hash_without_returning_key() {
    ...
    let stored_hash: String =
        sqlx::query_scalar("SELECT project_key_hash FROM insights_projects WHERE id = 'default'")
            .fetch_one(&pool)
            .await
            .expect("stored project key hash");
    assert_eq!(
        stored_hash,
        crate::features::settings::service::hash_project_key("rotated-project-key").expect("hash")
    );
...
async fn collection_policy_put_configures_a_clean_install_without_sql() {
    ...
    assert_eq!(
        stored.1,
        crate::features::settings::service::hash_project_key("fresh-project-key").expect("hash")
    );
}
...
async fn policy_changes_reject_old_tracking_without_new_rows() {
    ...
    for (label, update, key) in [
        ...
        let updated = app
            .clone()
            .oneshot(managed_json_request(Method::PUT, "/api/insights/collection-policy", update))
            .await
...
fn json_request(method: Method, uri: &str, access: DelegatedAccess, body: Value) -> Request<Body> {
...
fn signed_request(method: Method, uri: &str, access: DelegatedAccess, body: Body) -> Request<Body> {
