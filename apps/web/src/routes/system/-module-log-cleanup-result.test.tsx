...
test("cleanup result renders exact success and partial summaries", () => {
    const success = CleanupResult({ result: { removed: [], retained: [], failures: [], partial: false } as never });
    expect(success.props.type).toBe("success");
    expect(success.props.message).toBe("清理完成");
    expect(nodes(success).some((node) => (node.type as { name?: string }).name === "FailureList")).toBeFalse();

    const partial = CleanupResult({ result: { removed: [{}, {}], retained: [{}], failures, partial: true } as never });
    expect(partial.props.type).toBe("warning");
    expect(partial.props.message).toBe("清理完成，但需要复核部分结果");
    expect(text(partial.props.description)).toContain("已删除 2 个文件，保留 1 个文件，失败 2 个文件。");
});
...
test("failure list keeps every failure text and stable key", () => {
    ...
    expect(items.map((item) => item.key)).toEqual(["monitor:monitor.old:changed", "reports:reports.old:unreadable"]);
    expect(items.map(text)).toEqual(["monitor / monitor.old: changed", "reports / reports.old: unreadable"]);
});
