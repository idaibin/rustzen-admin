...
function tailNodes(tree: ReactElement) {
    const tail = nodes(tree).find(
        (node) => (node.type as { name?: string }).name === "TailContent",
    )!;
...
test("tail drawer renders pending, absent, empty, truncated, and older cursor states", () => {
    expect(
        nodes(render({ query: query({ data: undefined, isPending: true }) })!).some(
            (node) => node.props.kind === "loading",
        ),
    ).toBeTrue();
    expect(
        nodes(render({ query: query({ data: undefined }) })!).some(
            (node) => node.props.kind === "empty",
        ),
    ).toBeTrue();
    expect(
        tailNodes(
            render({ query: query({ data: { ...data, content: "", nextCursor: undefined } }) })!,
        ).some((node) => node.props.kind === "empty"),
    ).toBeTrue();
    let cursor = "";
    const tree = render({
    ...
    const button = content.find(
        (node) => node.props.loading === true && typeof node.props.onClick === "function",
    )!;
...
test("tail drawer keeps closed, absent-content, and exact bounded-content states distinct", () => {
    ...
    const absent = nodes(render({ query: query({ data: undefined }) })!).find(
        (node) => node.props.kind === "empty",
    )!;
    expect(absent.props.title).toBe("未找到日志内容");

    const emptyTree = render({
        query: query({ data: { ...data, content: "", nextCursor: undefined } }),
    })!;
    const empty = tailNodes(emptyTree).find((node) => node.props.kind === "empty")!;
    expect(empty.props.title).toBe("日志文件为空");
    expect(
        tailNodes(emptyTree).some((node) => typeof node.props.onClick === "function"),
    ).toBeFalse();

    const contentTree = render({
        query: query({ data: { ...data, content: "exact line", nextCursor: undefined } }),
    })!;
    const content = tailNodes(contentTree).find(
        (node) => node.props["data-testid"] === "module-log-tail-content",
    )!;
    expect(content).toBeDefined();
    const pre = nodes(content).find((node) => node.type === "pre")!;
    expect(pre.props.children).toBe("exact line");
    expect(
        tailNodes(contentTree).some((node) => typeof node.props.onClick === "function"),
    ).toBeFalse();
});
