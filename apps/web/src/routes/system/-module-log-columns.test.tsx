...
const readable = { module: "admin", date: "2026-09-11", fileName: "admin.2026-09-11", sizeBytes: 1536, modifiedAt: "invalid", readable: true, active: true };
...
test("module log columns retain metadata, status, and view-action contracts", () => {
    let opened: unknown;
    const columns = getModuleLogColumns((record) => { opened = record; });
    expect(columns.map((column) => [column.title, column.dataIndex, column.key, column.width, column.fixed, column.ellipsis])).toEqual([
    ...
    expect([file.props.children[1].props.type, file.props.children[1].props.children]).toEqual(["secondary", readable.date]);
    ...
    expect([active.props.children[0].props.color, active.props.children[0].props.children]).toEqual(["green", "可读"]);
    expect([active.props.children[1].props.color, active.props.children[1].props.children]).toEqual(["orange", "当前文件"]);
    const inactive: any = render(columns[3], unreadable);
    expect([inactive.props.children[0].props.color, inactive.props.children[0].props.children]).toEqual(["red", "不可读"]);
    ...
    for (const [action, record, disabled] of [[readAction, readable, false], [blockedAction, unreadable, true]] as const) {
        expect([action.props["data-testid"], action.props.type, action.props.size, action.props.disabled, action.props.children]).toEqual([`module-log-tail-${record.module}-${record.date}`, "link", "small", disabled, "查看"]);
