    ...
    const columns: ProColumns<Task.Item>[] = useMemo(
        () => [
            ...
            },
            {
                ...
                render: (_: unknown, row: Task.Item) => (
                    <TaskStatusBadge
                        testId={`maintenance-task-status-${row.taskKey}`}
                        status={row.running ? "running" : row.lastStatus}
                    />
                ),
            },
            {
            ...
            },
            {
                ...
                width: 120,
                render: (_: unknown, row: Task.Item) => (
                    <Typography.Text ellipsis={{ tooltip: row.lastErrorMessage || "-" }}>
...
function TaskContent({
    ...
    return (
        <TaskCard title={title} description={description}>
            <DataTableShell
                fill
                ariaLabel={t("维护任务", "Maintenance tasks table")}
                testId="maintenance-task-table"
            >
                <ProTable<Task.Item>
                    rowKey="taskKey"
                    columns={columns}
