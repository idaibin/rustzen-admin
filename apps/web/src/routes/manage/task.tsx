...
function TaskContent({ rows, columns, loading, pending, error, refetch }: { rows: Task.Item[]; columns: ProColumns<Task.Item>[]; loading: boolean; pending: boolean; error: unknown; refetch: () => void }) {
    ...
    return <TaskCard title={title} description={description}><DataTableShell fill ariaLabel={t("维护任务", "Maintenance tasks table")} testId="maintenance-task-table"><ProTable<Task.Item> rowKey="taskKey" columns={columns} dataSource={rows} loading={loading} search={false} options={false} pagination={false} scroll={{ x: "max-content", y: "100%" }} toolBarRender={false} tableAlertOptionRender={false} rowSelection={false} locale={{ emptyText: <DataState kind="empty" title={t("暂无定时任务", "No scheduled tasks")} /> }} /></DataTableShell></TaskCard>;
}
