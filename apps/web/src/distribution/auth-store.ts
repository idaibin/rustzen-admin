...
export function createDistributionAuthStore(routePermissions: Record<string, string>) {
    const getRouteCapabilityCodes = (pathname: string): string[] => {
        const capability = routePermissions[pathname];
    ...
    return { getRouteCapabilityCodes, useAuthStore };
}
